...
def extract_all_names_and_births(text):
    """
    등기부등본 텍스트의 '주요 등기사항 요약' 부분에서
    모든 등기명의인(소유자, 공유자)의 이름과 생년월일을 추출합니다.
    
    Args:
        text (str): PDF에서 추출한 전체 텍스트
    
    Returns:
        list: (이름, 생년월일) 튜플의 리스트. 예: [('전정미', '650804'), ('한동복', '600715')]
    """
    # 1. '주요 등기사항 요약' 섹션만 잘라내기
...
# --- 테스트용 예시 ---
# 제공된 OCR 텍스트의 마지막 페이지(주요 등기사항 요약)를 기반으로 테스트 데이터를 만듭니다.
sample_text = """
주요 등기사항 요약(참고용)
[주의사항]
본 주요 등기사항 요약은 ...
...
전정미 (공유자)  650804-*******  3분의 1     서울특별시 강남구 테헤란로4길 46, 101동 1402호  8
한동복 (공유자)  600715-*******  3분의 2     서울특별시 강남구 테헤란로4길 46, 101동 1402호  8
...
12       근저당권설정    2024년3월27일 제53072호  채권최고액 금540,000,000원      전정미 등
"""

# 함수 실행
owners_info = extract_all_names_and_births(sample_text)
print(owners_info)
# 예상 출력: [('전정미', '650804'), ('한동복', '600715')]
